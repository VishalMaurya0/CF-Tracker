...
export default function BacklogPanel({ API, user, onAddedToQueue }) {
  ...
  const load = async () => {
    ...
    try {
      const res = await axios.get(`${API}/api/backlog?handle=${user.handle}`);
      setData(res.data);
    ...
    <div style={{ animation:"fadeIn .3s ease" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:20 }}>
        <div>
          ...
          <p style={{ color:"#555", fontSize:13 }}>Problems your friends solved that you haven't — add any to your priority queue.</p>
        </div>
        <button onClick={load} disabled={loading}
          ...
          {loading ? <><span className="spinner"/>Fetching...</> : "Fetch backlog"}
        </button>
      </div>
            ...
            {filtered.map(p => {
              ...
              return (
                <div key={key} style={{ background:"#111", border:`1px solid ${p.nearMyRating?"#1a1a2e":"#141414"}`, borderRadius:10, padding:"12px 16px", display:"flex", alignItems:"center", gap:12 }}>
                  {/* Rating */}
                  <div style={{ minWidth:44, textAlign:"center", fontSize:12, fontWeight:600, padding:"3px 6px", borderRadius:6,
