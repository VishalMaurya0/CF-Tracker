...
export default function SetupScreen({ API, onDone }) {
  ...
  const go = async () => {
    ...
    try {
      ...
      // Fetch full profile to get existing friends from DB
      const profileRes = await axios.get(`${API}/api/profile?handle=${handle.trim()}`);
      const existingFriends = profileRes.data?.settings?.friendHandles || [];

      onDone({
        handle: handle.trim(),
        practiceRating: rating ? Number(rating) : cfUser.rating || 1200,
        currentRating: cfUser.rating || 0,
        friendHandles: existingFriends,
      });
    } catch (e) {
